...
describe('CDK App Entry Point', () => {
  ...
  describe('Domain Resolution', () => {
    it('uses default domain when no context provided', () => {
      ...
      expect(domain).toBe('k3frame.com');
    });
  ...
  describe('CoreParamPrefix Resolution', () => {
    ...
    it('uses default coreParamPrefix for k3frame.com', () => {
      const app = new cdk.App();
      const domain = 'k3frame.com';
      const coreParamPrefixValue = coreParamPrefix(domain);
      
      expect(coreParamPrefixValue).toBe('/k3frame/core');
    });
  });
  ...
  describe('Stack Name Derivation', () => {
    ...
    it('derives correct stack name for k3frame.com', () => {
      const stackName = toMailserverInstanceStackName('k3frame.com');
      expect(stackName).toBe('k3frame-com-mailserver-instance');
    });
  });
