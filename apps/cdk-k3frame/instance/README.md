...
```bash
pnpm nx build cdk-k3frame-instance
```
...
CDK_DEFAULT_ACCOUNT=<account-id> CDK_DEFAULT_REGION=us-east-1 \
  pnpm nx run cdk-k3frame-instance:synth
```
...
```bash
CDK_DEFAULT_ACCOUNT=<account-id> CDK_DEFAULT_REGION=us-east-1 \
  pnpm nx run cdk-k3frame-instance:diff
```
...
```bash
CDK_DEFAULT_ACCOUNT=<account-id> CDK_DEFAULT_REGION=us-east-1 \
  pnpm nx run cdk-k3frame-instance:destroy
```
...
```bash
# Fetch status checks
pnpm nx run cdk-k3frame-instance:admin:miab:status-check

# Save to JSON file for analysis
OUTPUT_FILE=status-report.json pnpm nx run cdk-k3frame-instance:admin:miab:status-check

# Verbose output
VERBOSE=1 pnpm nx run cdk-k3frame-instance:admin:miab:status-check
```
...
- **Disk space low**: Run cleanup: `pnpm nx run cdk-k3frame-instance:admin:cleanup:disk-space`
