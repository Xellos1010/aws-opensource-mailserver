...
describe('Deploy Schedule Validation E2E', () => {
  ...
  describe('EventBridge Schedule Expression Validation', () => {
    it('validates EventBridge cron schedule has correct format (6 fields)', () => {
      ...
      try {
        execSync(`FEATURE_CDK_EMCNOTARY_STACKS_ENABLED=1 DOMAIN=test.example.com pnpm nx run cdk-emcnotary-instance:synth`, {
          stdio: 'pipe',
          cwd: process.cwd(),
        });
    ...
    it('validates custom schedule expression format', () => {
      ...
      try {
        execSync(
          `FEATURE_CDK_EMCNOTARY_STACKS_ENABLED=1 DOMAIN=test.example.com INSTANCE_NIGHTLY_REBOOT_SCHEDULE="0 9 * * ? *" pnpm nx run cdk-emcnotary-instance:synth`,
          {
            stdio: 'pipe',
            cwd: process.cwd(),
          }
