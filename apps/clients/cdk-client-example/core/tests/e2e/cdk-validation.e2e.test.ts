...
describe('CDK Synthesis E2E', () => {
  const cdkOutDir = join(process.cwd(), 'dist/apps/clients/cdk-client-example/core/cdk.out');
  const testStackName = 'example-com-mailserver-core';

  beforeAll(() => {
    // Build the project first (this creates the dist directory)
    try {
      execSync('pnpm nx build cdk-client-example-core', {
        stdio: 'pipe',
        cwd: process.cwd(),
      });
    } catch (error) {
    ...
    try {
      const distDir = join(process.cwd(), 'dist/apps/clients/cdk-client-example/core');
  ...
  it('generates valid CloudFormation templates', () => {
    ...
    try {
      ...
      execSync('pnpm nx run cdk-client-example-core:synth', {
        stdio: 'pipe',
        cwd: process.cwd(),
        env: {
          ...process.env,
  ...
  it('template parameters have correct types and defaults', () => {
    ...
    expect(template.Parameters.DomainName).toHaveProperty('Default', 'example.com');
