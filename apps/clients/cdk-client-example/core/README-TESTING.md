...
### Run All Tests
```bash
pnpm nx test cdk-client-example-core
```

### Run Unit Tests Only
```bash
pnpm nx test cdk-client-example-core --testPathPattern=__tests__
```

### Run Integration Tests Only
```bash
pnpm nx test cdk-client-example-core --testPathPattern=__it__
```

### Run E2E Tests Only
```bash
pnpm nx test cdk-client-example-core --testPathPattern=e2e
```

### Run with Coverage
```bash
pnpm nx test cdk-client-example-core --coverage
```
...
### For E2E Tests
- CDK stack must be built: `pnpm nx build cdk-client-example-core`
- CDK synthesis must succeed: `pnpm nx run cdk-client-example-core:synth`
- Feature flag must be set: `FEATURE_CDK_EMC_NOTARY_EXAMPLE_STACKS_ENABLED=1`
...
pnpm nx test cdk-client-example-core --testPathPattern="__tests__|__it__"

# Run only E2E tests
pnpm nx test cdk-client-example-core --testPathPattern="e2e"

# Run specific test file
pnpm nx test cdk-client-example-core --testFile=main.spec.ts
```
...
### E2E tests fail
- Ensure CDK stack is built: `pnpm nx build cdk-client-example-core`
- Run synthesis manually: `pnpm nx run cdk-client-example-core:synth`
- Check that `FEATURE_CDK_EMC_NOTARY_EXAMPLE_STACKS_ENABLED=1` is set
