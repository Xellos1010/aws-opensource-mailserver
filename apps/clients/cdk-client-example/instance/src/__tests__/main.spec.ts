...
describe('CDK App Entry Point', () => {
  ...
  describe('CoreParamPrefix Resolution', () => {
    ...
    it('uses default coreParamPrefix for example.com', () => {
      const app = new cdk.App();
      const domain = 'example.com';
      const coreParamPrefixValue = coreParamPrefix(domain);
      
      expect(coreParamPrefixValue).toBe('/example/core');
    });
  });
  ...
  describe('Stack Name Derivation', () => {
    ...
    it('derives correct stack name for example.com', () => {
      const stackName = toMailserverInstanceStackName('example.com');
      expect(stackName).toBe('example-com-mailserver-instance');
    });
  });
