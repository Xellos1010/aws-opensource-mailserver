...
- **Stack Name**: `example-com-mailserver-instance` (format: `{domain-tld}-mailserver-instance`)
...
Mail stack procedures and Nx targets are documented under [docs/public/mail-server-operations.md](../../../docs/public/mail-server-operations.md) and [docs/public/nx-cdk-reference.md](../../../docs/public/nx-cdk-reference.md).
...
For sample mailserver, the `coreParamPrefix` is `/example/core`.
...
- `apps/clients/cdk-client-example/observability-maintenance`

The instance stack publishes this SSM metadata contract consumed by that stack:

- `/example/instance/instanceId`
- `/example/instance/instanceDns`
- `/example/instance/stackName`
...
- **CDK Context**: `cdk deploy --context domain=example.org`
- **Environment Variables**: `DOMAIN=example.org`
...
```bash
pnpm nx build cdk-client-example-instance
```
...
`cdk-client-example-observability-maintenance`, not by the instance stack.
...
```bash
# Fetch status checks
pnpm nx run cdk-client-example-instance:admin:miab:status-check

# Save to JSON file for analysis
OUTPUT_FILE=status-report.json pnpm nx run cdk-client-example-instance:admin:miab:status-check

# Verbose output
VERBOSE=1 pnpm nx run cdk-client-example-instance:admin:miab:status-check
```
...
- **`FEATURE_CDK_EMC_NOTARY_EXAMPLE_STACKS_ENABLED`**: Controls CDK stack deployment (default: `0`, set to `1` to enable)
