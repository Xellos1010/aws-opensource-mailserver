...
describe('Deploy Operation E2E', () => {
  ...
  it('synth succeeds before deploy', () => {
    ...
    execSync('pnpm nx build cdk-client-example-instance', {
      stdio: 'pipe',
      cwd: process.cwd(),
    });
    ...
    const templatePath = join(
      process.cwd(),
      'dist/apps/clients/cdk-client-example/instance/cdk.out',
      'example-com-mailserver-instance.template.json'
    );
    ...
    try {
      execSync('pnpm nx run cdk-client-example-instance:synth', {
        stdio: 'pipe',
        timeout: 60000,
        cwd: process.cwd(),
        env: {
  ...
  it('deploy command structure is valid', () => {
    ...
    const deployCmd = 
      'FEATURE_CDK_EMC_NOTARY_EXAMPLE_STACKS_ENABLED=1 node ../../../../../dist/libs/infra/config-loader/bin/cdk-deploy.js deploy --require-approval never';
  ...
  it('validates core stack SSM parameters exist (prerequisite check)', () => {
    ...
    // - /example/core/domainName
    // - /example/core/backupBucket
    // - /example/core/nextcloudBucket
    // - /example/core/alarmsTopicArn
    // - /example/core/eipAllocationId
