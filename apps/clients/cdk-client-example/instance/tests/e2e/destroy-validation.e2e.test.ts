...
describe('Destroy Operation E2E', () => {
  ...
  it('destroy command structure is valid', () => {
    ...
    const destroyCmd = 
      'FEATURE_CDK_EMC_NOTARY_EXAMPLE_STACKS_ENABLED=1 node ../../../../../dist/libs/infra/config-loader/bin/cdk-deploy.js destroy --force';
  ...
  it('validates feature flag is set for destroy', () => {
    // Destroy command should require FEATURE_CDK_EMC_NOTARY_EXAMPLE_STACKS_ENABLED=1
    const projectJson = require('../../project.json');
    const destroyCommand = projectJson.targets.destroy.options.command;
    
    expect(destroyCommand).toContain('FEATURE_CDK_EMC_NOTARY_EXAMPLE_STACKS_ENABLED=1');
  });
  ...
  it('validates destroy does not affect core stack (EIP remains)', () => {
    ...
    expect(destroyCommand).not.toContain('cdk-client-example-core');
  });
});
