...
Commands:
  ...
  stack:core:deploy [domain]        # Deploy core stack (default: example.com)
  stack:instance:deploy [domain]   # Deploy instance stack (default: example.com)
  stack:observability:deploy [domain] # Deploy observability-maintenance stack
  stack:core:destroy [domain]      # Destroy core stack (default: example.com)
  stack:instance:destroy [domain]   # Destroy instance stack (default: example.com)
  stack:observability:destroy [domain] # Destroy observability-maintenance stack
  stack:core:diff [domain]          # Show core stack diff (default: example.com)
  stack:instance:diff [domain]      # Show instance stack diff (default: example.com)
  ...
  instance:bootstrap [domain]       # Bootstrap MIAB via SSM (default: example.com)

Env:
  FEATURE_CDK_MAILSERVER_DEMO_ENABLED=1 required for stack operations
...
`);
  ...
  switch (cmd) {
    ...
    case 'stack:core:deploy': {
      ...
      execSync(`pnpm nx run cdk-client-example-core:deploy`, {
        stdio: 'inherit',
    ...
    case 'stack:instance:deploy': {
      ...
      // Use execSync to run the Nx target
      const { execSync } = await import('child_process');
      execSync(`pnpm nx run cdk-client-example-instance:deploy`, {
        stdio: 'inherit',
        env: { ...process.env, DOMAIN: domain }
      });
    ...
    case 'stack:observability:deploy': {
      ...
      const { execSync } = await import('child_process');
      execSync(`pnpm nx run cdk-mailexample-observability-maintenance:deploy`, {
        stdio: 'inherit',
        env: { ...process.env, DOMAIN: domain }
      });
    ...
    case 'stack:core:destroy': {
      ...
      const { execSync } = await import('child_process');
      execSync(`pnpm nx run cdk-client-example-core:destroy`, {
        stdio: 'inherit',
        env: { ...process.env, DOMAIN: domain }
      });
    ...
    case 'stack:instance:destroy': {
      ...
      const { execSync } = await import('child_process');
      execSync(`pnpm nx run cdk-client-example-instance:destroy`, {
        stdio: 'inherit',
        env: { ...process.env, DOMAIN: domain }
      });
    ...
    case 'stack:observability:destroy': {
      ...
      const { execSync } = await import('child_process');
      execSync(`pnpm nx run cdk-mailexample-observability-maintenance:destroy`, {
        stdio: 'inherit',
        env: { ...process.env, DOMAIN: domain }
      });
    ...
    case 'stack:core:diff': {
      ...
      const { execSync } = await import('child_process');
      execSync(`pnpm nx run cdk-client-example-core:diff`, {
        stdio: 'inherit',
        env: { ...process.env, DOMAIN: domain }
      });
      break;
    ...
    case 'stack:instance:diff': {
      ...
      const { execSync } = await import('child_process');
      execSync(`pnpm nx run cdk-client-example-instance:diff`, {
        stdio: 'inherit',
        env: { ...process.env, DOMAIN: domain }
      });
      break;
    ...
    case 'stack:observability:diff': {
      ...
      const { execSync } = await import('child_process');
      execSync(`pnpm nx run cdk-mailexample-observability-maintenance:diff`, {
        stdio: 'inherit',
        env: { ...process.env, DOMAIN: domain }
      });
      break;
    ...
    case 'instance:bootstrap': {
      ...
      execSync(`pnpm nx run cdk-client-example-instance:admin:mailboxes:permissions:repair`, {
        stdio: 'inherit',
        env: { ...process.env, DOMAIN: domain }
      });
