...
### Sample mailserver backup

```bash
# Using domain-specific target
pnpm nx run admin-dns-backup:backup:mailexample

# Using hierarchical app path target
pnpm nx run admin-dns-backup:backup:apps:cdk-client-example
```

Both automatically set:
- `AWS_PROFILE=your-aws-profile`
- `AWS_REGION=us-east-1`
- `APP_PATH=apps/clients/cdk-client-example` (for automatic stack discovery)
