...
export type StackInfoConfig = {
  stackName?: string;
  domain?: string;
  appPath?: string; // e.g., "apps/clients/cdk-client-example"
  region?: string;
  profile?: string;
};
 ...
 * Examples:
 * - "apps/clients/cdk-client-example" -> "example.com"
 * - legacy stack name hints -> "example.com"
 * - "example-com-mailserver-core" -> "example.com"
 * 
...
export function resolveDomain(
  ...
  if (appPath) {
    // Extract domain from app path (domainMap keys match app folder slug after cdk- prefix):
    // "apps/clients/cdk-client-example/core" -> "example.com"
    // "apps/clients/cdk-client-example/instance" -> "example.com"
    // "apps/clients/cdk-client-example" -> "example.com"
    ...
    // Convert kebab-case app slug to apex domain (demo / sample apps).
    const domainMap: Record<string, string> = {
      'client-example': 'example.com',
      'emc-notary-example': 'example.com',
      'emc-notary': 'example.com',
      'mailexample': 'example.com',
      'sampletenant': 'example.org',
      'sample-net': 'example.net',
    };
  ...
  if (stackName) {
    ...
    try {
      ...
      // "example-com-mailserver" or "example-com-mailserver-core" -> "example.com"
 ...
 * Examples:
 * - "example.com" + "core" -> "example-com-mailserver-core"
 * - "example.com" + "instance" -> "example-com-mailserver-instance"
 * - "apps/clients/cdk-client-example/core" -> "example-com-mailserver-core"
 * 
 * @param domain - Domain name (e.g., "example.com")
 * @param appPath - App path (e.g., "apps/clients/cdk-client-example/core")
...
export async function getStackInfo(
  ...
  const profile = config.profile || process.env['AWS_PROFILE'] || 'your-aws-profile';
 ...
 * Example: "apps/clients/cdk-client-example" -> StackInfo
 */
