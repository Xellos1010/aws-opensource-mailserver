...
async function main() {
  ...
  if (!domain && !appPath) {
    ...
    console.error('   or: APP_PATH=apps/clients/cdk-client-example/instance provision-instance [--skip-ssh] [--skip-ses-dns]');
    console.error('Example: provision-instance --domain example.com --skip-ssh');
    process.exit(1);
  }
  ...
  const result = await provisionInstance({
    ...
    profile: process.env['AWS_PROFILE'] || 'your-aws-profile',
    skipSsh,
    skipSesDns,
  });
