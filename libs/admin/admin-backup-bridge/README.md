...
### sample mailserver Backup (Recommended)

```bash
# Using hierarchical app path target
pnpm nx run admin-backup-bridge:backup:apps:cdk-client-example

# Using domain-specific target
pnpm nx run admin-backup-bridge:backup:mailexample
```
...
APP_PATH=apps/clients/cdk-client-example pnpm nx run admin-backup-bridge:backup

# Using domain
DOMAIN=example.com pnpm nx run admin-backup-bridge:backup

# Using explicit stack name
STACK_NAME=example-com-mailserver pnpm nx run admin-backup-bridge:backup
```
...
```bash
# DNS only
SKIP_MAIL=1 pnpm nx run admin-backup-bridge:backup:apps:cdk-client-example

# Mail only
SKIP_DNS=1 pnpm nx run admin-backup-bridge:backup:apps:cdk-client-example
```
...
```json
{
  ...
  "stackInfo": {
    "stackName": "example-com-mailserver",
    "domain": "example.com",
...
```bash
# Full backup for sample mailserver
pnpm nx run admin-backup-bridge:backup:apps:cdk-client-example
