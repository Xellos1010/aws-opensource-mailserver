...
const backupBaseDir = 'Archive/backups/example.com/mailboxes';
const backupFolders = findBackupFolders(backupBaseDir);

// Or specify folders manually
const backupFolders = [
  'Archive/backups/example.com/mailboxes/mailboxes-backup-20250923_201744',
  'Archive/backups/example.com/mailboxes/mailboxes-backup-20250923_195631',
  'Archive/backups/example.com/mailboxes/mailboxes-backup-20250915_000853',
];

// Aggregate users and emails
const result = await aggregateBackups(backupFolders, 'example.com');
...
nx run cdk-client-example-instance:admin:mailboxes:restore-aggregated
...
BACKUP_BASE_DIR="Archive/backups/example.com/mailboxes" \
nx run cdk-client-example-instance:admin:mailboxes:restore-aggregated

# Dry run
DRY_RUN=1 nx run cdk-client-example-instance:admin:mailboxes:restore-aggregated
```
...
```bash
MASTER_BACKUP_DIR="Archive/backups/example.com/mailboxes/master" \
nx run cdk-client-example-instance:admin:mailboxes:sync-master

# Dry run
MASTER_BACKUP_DIR="Archive/backups/example.com/mailboxes/master" \
DRY_RUN=1 nx run cdk-client-example-instance:admin:mailboxes:sync-master
```
...
```bash
MASTER_BACKUP_DIR="Archive/backups/example.com/mailboxes/master" \
nx run cdk-client-example-instance:admin:mailboxes:archive-master

# Specify archive base directory
MASTER_BACKUP_DIR="Archive/backups/example.com/mailboxes/master" \
ARCHIVE_BASE_DIR="Archive/backups/example.com/mailboxes" \
nx run cdk-client-example-instance:admin:mailboxes:archive-master
...
```
Archive/backups/example.com/mailboxes/
