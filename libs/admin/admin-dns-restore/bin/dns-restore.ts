...
async function main() {
  ...
  if (!backupFile) {
    ...
    console.error('\nExamples:');
    console.error('  BACKUP_FILE=Archive/backups/example.org/dns/dns-backup-20250915-120236.json HOSTED_ZONE_ID=Z123456789 node dns-restore.mjs');
    console.error('  BACKUP_FILE=backup.json APP_PATH=apps/clients/cdk-client-example node dns-restore.mjs');
    console.error('  BACKUP_FILE=backup.json APP_PATH=apps/clients/cdk-client-example DRY_RUN=1 node dns-restore.mjs');
    process.exit(1);
  }
