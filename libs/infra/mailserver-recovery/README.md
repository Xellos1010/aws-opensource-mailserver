...
Reusable CDK constructs for mailserver recovery system ported from `docs/public`. Provides **83-95% faster recovery times** (30-90 seconds vs 5-10 minutes) by using progressive recovery methods that avoid instance reboots when possible.
...
- Alarm → Lambda invoke permissions (critical fix from legacy-mailstack)
...
This library is ported from `docs/public` recovery system, which was proven in production with:
...
- `.cursor/plans/REQUIREMENTS_AUDIT_legacy-mailstack_recovery_system.md` - Requirements audit
- `.cursor/plans/IMPLEMENTATION_SUMMARY_legacy-mailstack_recovery.md` - Implementation summary
- `docs/public/COMPLETE-RECOVERY-SYSTEM.md` - Original system documentation
