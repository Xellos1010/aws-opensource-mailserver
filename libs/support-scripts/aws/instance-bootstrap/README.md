...
```bash
# Bootstrap default domain (example.com)
FEATURE_INSTANCE_BOOTSTRAP_ENABLED=1 DOMAIN=example.com \
  pnpm nx run ops-runner:instance:bootstrap

# Dry run
FEATURE_INSTANCE_BOOTSTRAP_ENABLED=1 DOMAIN=example.com DRY_RUN=1 \
  pnpm nx run ops-runner:instance:bootstrap

# Different domain
FEATURE_INSTANCE_BOOTSTRAP_ENABLED=1 DOMAIN=example.org \
  pnpm nx run ops-runner:instance:bootstrap
```
...
- `example.com` → `example-com-mailserver-instance`
- `example.org` → `example-org-mailserver-instance`
...
The core stack must provide these SSM parameters under `/mailexample/core/`:
- `domainName` - Domain name
...
- [Local operations](../../../../docs/public/local-operations.md)
- [Mail server operations bible](../../../../docs/public/mail-server-operations.md)
- [Instance stack README](../../../../apps/clients/cdk-client-example/instance/README.md)
