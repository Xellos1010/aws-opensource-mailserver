...
export interface BootstrapOptions {
  /** Domain name (e.g., "example.com") - used to derive stack name if stackName not provided */
  domain?: string;
  /** App path (e.g., "apps/clients/cdk-client-example/instance") - used to derive stack name if stackName not provided */
  appPath?: string;
  /** Explicit stack name (overrides domain/appPath-derived) */
  stackName?: string;
  /** AWS region (default: "us-east-1") */
  region?: string;
  /** AWS profile for credentials (e.g., "your-aws-profile") */
  profile?: string;
...
async function describeInstanceStack(
  ...
  if (!resolvedDomainName) {
    // Try to derive from stack name: example-com-mailserver-instance -> example.com
    // or example-com-mailserver-instance -> example.com
...
async function readCoreParams(
  ...
  // Determine core param prefix from domain (e.g., /mailexample/core or /{domain}/core)
...
Example:
  MAILINABOX_VERSION=v73 pnpm nx run cdk-client-example-instance:admin:bootstrap-miab-ec2-instance`;
...
async function pollCommandStatus(
  ...
  while (true) {
    ...
    if (status === CommandStatus.FAILED || status === CommandStatus.CANCELLED) {
      ...
      console.error(`\n💡 Troubleshooting:`);
      console.error(`   1. Check full logs: pnpm nx run cdk-client-example-instance:admin:bootstrap:logs`);
      console.error(`   2. Check status: pnpm nx run cdk-client-example-instance:admin:bootstrap:status`);
      console.error(`   3. View CloudWatch: aws logs tail /aws/ssm/miab-bootstrap --follow`);
      console.error(`   4. If git/permission errors: pnpm nx run cdk-client-example-instance:admin:miab:cleanup`);
...
export async function bootstrapInstance(
  ...
  if (options.dryRun) {
    ...
    try {
      ...
      try {
        ...
        if (!lastStatus?.online && checked) {
          ...
          console.log(`      pnpm nx run cdk-client-example-instance:admin:fix-ssm-agent`);
    ...
    try {
      ...
      const profile = options.profile || process.env.AWS_PROFILE || 'your-aws-profile';
      
      const sshTestProcess = spawn('pnpm', [
        'nx',
        'run',
        'cdk-client-example-instance:admin:ssh:test',
      ], {
        env: {
          ...process.env,
          AWS_PROFILE: profile,
          AWS_REGION: region,
          DOMAIN: domain,
          APP_PATH: 'apps/clients/cdk-client-example/instance',
        },
