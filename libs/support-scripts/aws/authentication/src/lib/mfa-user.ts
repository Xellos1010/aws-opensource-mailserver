...
function loadConfig(): AuthConfig {
  return {
    mfaArn: (() => {
      const arn = process.env.MFA_DEVICE_ARN;
      if (!arn) throw new Error('MFA_DEVICE_ARN environment variable is required');
      return arn;
    })(),
    sourceProfile: process.env.SOURCE_PROFILE ?? 'your-source-profile',
    targetProfile: process.env.TARGET_PROFILE ?? 'your-aws-profile',
