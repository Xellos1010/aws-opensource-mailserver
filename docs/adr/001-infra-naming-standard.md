...
- Legacy fallbacks allowed non-TLD instance stack names (e.g., `example-com-mailserver-instance` vs `example-com-mailserver-instance`)
...
- **Core stacks**: `{domain-tld}-mailserver-core` (e.g., `example-com-mailserver-core`)
- **Instance stacks**: `{domain-tld}-mailserver-instance` (e.g., `example-com-mailserver-instance`)
...
- **Risk**: SSM parameter paths remain stable, so consumers unaffected
  - **Mitigation**: Verified - SSM paths use domain name without TLD (e.g., `/mailexample/core`)
...
2. Refactored all call sites:
   - `apps/clients/cdk-client-example/core/src/main.ts`
   - `apps/clients/cdk-client-example/instance/src/main.ts`
...
- [Nx / CDK reference (public)](../public/nx-cdk-reference.md)
