...
async function handleSuccessResponse(
  ...
  if (result.httpCode === 200) {
    ...
    console.log('\n💡 Next steps:');
    console.log(`   1. Verify certificates: pnpm nx run cdk-client-example-instance:admin:ssl:status`);
    ...
    console.log(`   2. Verify certificates: pnpm nx run cdk-client-example-instance:admin:ssl:status`);
...
async function provisionSslCertificatesViaApi(
  ...
  try {
    ...
    if (!success) {
      ...
      console.error(`   4. Try finding the endpoint: pnpm nx run cdk-client-example-instance:admin:ssl:find-endpoint`);
    ...
    if (error instanceof Error) {
      ...
      if (error.message.includes('ECONNREFUSED') || error.message.includes('ENOTFOUND')) {
        ...
        console.error('\n💡 Troubleshooting:');
        console.error(`   1. Verify admin credentials: pnpm nx run cdk-client-example-instance:admin:credentials`);
