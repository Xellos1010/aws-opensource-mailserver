...
Options:
  --domain, -d <domain>           Domain name (e.g., example.org)
  --app-path <path>               App path (default: APP_PATH or apps/clients/cdk-client-example/instance)
...
Examples:
  ...
  pnpm exec tsx --tsconfig tools/tsconfig.json tools/backup-and-cleanup-mailserver.cli.ts --domain example.net

  # Legacy stack run (legacy-mailstack style)
  pnpm exec tsx --tsconfig tools/tsconfig.json tools/backup-and-cleanup-mailserver.cli.ts --domain example.org --stack-name example-org-mailserver

  # Dry run preview
  pnpm exec tsx --tsconfig tools/tsconfig.json tools/backup-and-cleanup-mailserver.cli.ts --domain example.org --stack-name example-org-mailserver --dry-run
`);
  process.exit(code);
}
