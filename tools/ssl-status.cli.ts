...
async function checkSslStatus(options: SslStatusOptions): Promise<void> {
  ...
  try {
    ...
    if (!keyPath) {
      ...
      if (!agentAvailable) {
      throw new Error(
          'SSH key not found and SSH agent not available. Run: pnpm nx run cdk-client-example-instance:admin:ssh:setup'
      );
      }
    ...
    if (isLetsEncrypt && failed === 0) {
      ...
      console.log('\n   Run: pnpm nx run cdk-client-example-instance:admin:ssl:provision');
      process.exit(1);
    } else {
      console.log('❌ SSL certificates are NOT PROVISIONED');
      console.log('   Certificates need to be provisioned.\n');
      console.log('💡 Next steps:');
      console.log('   1. Provision certificates: pnpm nx run cdk-client-example-instance:admin:ssl:provision');
      console.log('   2. Wait 1-2 minutes for provisioning to complete');
      console.log('   3. Re-run this check: pnpm nx run cdk-client-example-instance:admin:ssl:status\n');
      process.exit(1);
    }
  } catch (error) {
