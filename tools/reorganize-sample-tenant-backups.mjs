...
/**
 * Reorganize archived sampletenant backups to match the structure
...
const ARCHIVE_DIR = path.join(workspaceRoot, 'Archive/backups/example.org');
const DOMAIN = 'sampletenant';
...
async function getHostedZoneId() {
  ...
  try {
    ...
    const result = execSync(
      `AWS_PROFILE=your-aws-profile AWS_REGION=us-east-1 aws route53 list-hosted-zones --query "HostedZones[?Name=='example.org.'].Id" --output text 2>/dev/null`,
  ...
  try {
    const { execSync } = await import('child_process');
    const result = execSync(
      'pnpm nx run admin-stack-info:get:sampletenant 2>&1',
      { encoding: 'utf-8', cwd: workspaceRoot }
    );
...
async function reorganizeDnsBackups(hostedZoneId) {
  ...
  for (const file of dnsFiles) {
    ...
    const newData = convertDnsBackup(oldData, hostedZoneId, 'example.org.');
    if (!newData) {
...
async function main() {
  console.log('Reorganizing sampletenant backups...\n');
