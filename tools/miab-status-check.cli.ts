...
function parseStatusChecks(rawOutput: string): StatusCheckResult[] {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    // Detect category headers (System, Network, Domain names like box.example.com, example.com)
...
async function fetchStatusChecks(options: StatusCheckOptions): Promise<StatusCheckReport> {
  ...
  if (!keyPath) {
    throw new Error('SSH key not found. Run: pnpm nx run cdk-client-example-instance:admin:ssh:setup');
  }
  ...
  if (!statusOutput || 
    ...
    if (miabDirCheck.output === 'NOT_FOUND') {
      throw new Error('Mail-in-a-Box is not installed. Run bootstrap first: pnpm nx run cdk-client-example-instance:admin:bootstrap-miab-ec2-instance');
    }
...
Options:
  ...
  --app-path PATH          App path (default: apps/clients/cdk-client-example/instance)
  --region REGION          AWS region (default: us-east-1)
  --profile PROFILE        AWS profile (default: your-aws-profile)
...
Examples:
  # Fetch status checks for example.com
  pnpm nx run cdk-client-example-instance:admin:miab:status-check

  # Save to file for analysis
  pnpm nx run cdk-client-example-instance:admin:miab:status-check -- --output status.json
