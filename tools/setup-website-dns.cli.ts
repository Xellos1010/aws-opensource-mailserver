 ...
 * 1. The root domain (e.g., example.net)
 * 2. The www subdomain (e.g., www.example.net)
 *
 * IP address can be:
 ...
 * - WEBSITE_PROFILE (default: sample-net) - for looking up website stack IP
 * - AWS_PROFILE (default: your-aws-profile) - for MIAB instance operations
 *
 * Usage:
 *   # With explicit IP:
 *   WEBSITE_IP=1.2.3.4 pnpm nx run cdk-client-example-instance:admin:dns:setup-website
 *
 *   # With CloudFormation stack (uses sample-net profile for website stack):
 *   WEBSITE_STACK_NAME=sample-net-react-webserver pnpm nx run cdk-client-example-instance:admin:dns:setup-website
 *
 *   # Dry run:
 *   WEBSITE_IP=1.2.3.4 DRY_RUN=1 pnpm nx run cdk-client-example-instance:admin:dns:setup-website
 */
...
interface SetupWebsiteDnsOptions {
  ...
  websiteProfile?: string; // AWS profile for website stack lookup (default: sample-net)
  appPath?: string;
  region?: string;
  profile?: string; // AWS profile for MIAB instance operations (default: your-aws-profile)
  dryRun?: boolean;
  verbose?: boolean;
}
...
async function setupWebsiteDns(options: SetupWebsiteDnsOptions): Promise<void> {
  ...
  const websiteProfile = options.websiteProfile || process.env.WEBSITE_PROFILE || 'sample-net';
  const domain = options.domain || process.env.DOMAIN || 'example.net';
  const appPath = options.appPath || process.env.APP_PATH || 'apps/clients/cdk-client-example/instance';
