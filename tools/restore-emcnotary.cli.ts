...
/**
 * Quick restore script for emcnotary.com
...
async function main(): Promise<void> {
  ...
  const APP_PATH = process.env.APP_PATH || 'apps/cdk-emc-notary/instance';
  
  if (!DOMAIN) {
    console.error('Error: DOMAIN environment variable is required');
    console.error('Usage: DOMAIN=emcnotary.com pnpm nx run cdk-emcnotary-instance:admin:restore:emcnotary');
    process.exit(1);
  }
  ...
  console.log('🔄 EMC Notary Restore');
