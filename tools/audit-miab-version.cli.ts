...
Example:
  MAILINABOX_VERSION=v73 pnpm nx run cdk-client-example-instance:admin:miab:audit`;
    ...
    if (!keyPath) {
      throw new Error(
        'SSH key not found. Run: pnpm nx run cdk-client-example-instance:admin:ssh:setup'
      );
    }
    ...
    if (needsUpdate && update) {
      ...
      if (!fetchResult.success) {
        ...
        if (!retryFetch.success) {
          throw new Error(
            ...
            `💡 Try running cleanup first: pnpm nx run cdk-client-example-instance:admin:miab:cleanup`
          );
        }
      }
      ...
      console.log(`   UPDATE=1 pnpm nx run cdk-client-example-instance:admin:miab:audit\n`);
    }
      ...
      if (usersResult.success && usersResult.output) {
        ...
        if (users.length > 0) {
          ...
          if (adminUser) {
            ...
            console.log(`   pnpm nx run cdk-client-example-instance:admin:credentials:create\n`);
          }
        } else {
    ...
    if (needsUpdate && !update) {
      ...
      console.log(`      pnpm nx run cdk-client-example-instance:admin:users:list\n`);
    }
