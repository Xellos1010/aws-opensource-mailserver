...
/**
 * Quick restore script for example.com
...
async function main(): Promise<void> {
  ...
  const APP_PATH = process.env.APP_PATH || 'apps/clients/cdk-client-example/instance';
  
  if (!DOMAIN) {
    console.error('Error: DOMAIN environment variable is required');
    console.error('Usage: DOMAIN=example.com pnpm nx run cdk-client-example-instance:admin:restore:mailexample');
    process.exit(1);
  }
  
  const MAILBOX_BACKUP_PATH = process.env.MAILBOX_BACKUP_PATH || `Archive/backups/${DOMAIN}/mailboxes/mailboxes-backup-20250923_195631`;
  const DNS_BACKUP_PATH = process.env.DNS_BACKUP_PATH || `Archive/backups/${DOMAIN}/dns/dns-backup-20250915-120038.json`;
  
  console.log('🔄 sample mailserver Restore');
