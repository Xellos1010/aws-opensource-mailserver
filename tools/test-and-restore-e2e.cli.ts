  ...
  if (!options.skipDeploy) {
    ...
    if (!dryRun) {
      ...
      const coreDeploy = spawn(
        'pnpm',
        ['nx', 'run', 'cdk-client-example-core:deploy'],
      ...
      await pollStackStatus('example-com-mailserver-core', region, profile);
      ...
      const instanceDeploy = spawn(
        'pnpm',
        ['nx', 'run', 'cdk-client-example-instance:deploy'],
        { stdio: 'inherit', shell: true }
      );
      ...
      await pollStackStatus('example-com-mailserver-instance', region, profile);
  ...
  if (bootstrapCheck.output === 'INCOMPLETE') {
    ...
    if (!dryRun) {
      const bootstrap = spawn(
        'pnpm',
        ['nx', 'run', 'cdk-client-example-instance:admin:bootstrap-miab-ec2-instance'],
        { stdio: 'inherit', shell: true }
      );
