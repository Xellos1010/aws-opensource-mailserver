...
async function fixSesIamPermissions(): Promise<void> {
  ...
  const userName = 'ses-smtp-user-example-com-mailserver-core';
  ...
  const policy = {
    ...
    Statement: [
      {
        Sid: 'AllowSendFromExampleDomain',
        Effect: 'Allow',
        Action: [
          'ses:SendRawEmail',
          'ses:SendEmail'
        ...
        Resource: [
          `arn:aws:ses:${region}:${accountId}:identity/example.com`,
          `arn:aws:ses:${region}:${accountId}:identity/*@example.com`
        ]
      }
    ]
  ...
  await iam.send(new PutUserPolicyCommand({
    UserName: userName,
    PolicyName: 'SES-SendEmail-mailexample-com',
    PolicyDocument: JSON.stringify(policy, null, 2)
  }));

  console.log('   ✅ Inline policy created: SES-SendEmail-mailexample-com\n');
  ...
  console.log('   - Send emails from example.com');
  console.log('   - Send emails from any @example.com address');
