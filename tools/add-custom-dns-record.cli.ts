 ...
 *
 * Usage:
 *   RECORD_NAME=www.example.net RECORD_TYPE=A RECORD_VALUE=3.211.200.169 pnpm exec tsx tools/add-custom-dns-record.cli.ts
 */
...
async function addCustomDnsRecord() {
  if (!recordName || !recordValue) {
    ...
    console.error('Usage: RECORD_NAME=www.example.net RECORD_TYPE=A RECORD_VALUE=3.211.200.169 pnpm exec tsx tools/add-custom-dns-record.cli.ts');
    process.exit(1);
  }
