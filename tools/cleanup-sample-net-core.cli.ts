...
/**
 * Cleanup script for sample-net core stack orphaned resources
 * 
 * This script removes all resources associated with the example-net-mailserver-core stack
...
const STACK_NAME = 'example-net-mailserver-core';
...
async function deleteSSMParameters(): Promise<void> {
  ...
  const paramPaths = [
    '/sample-net/core/domainName',
    '/sample-net/core/backupBucket',
    '/sample-net/core/nextcloudBucket',
    '/sample-net/core/alarmsTopicArn',
    '/sample-net/core/sesIdentityArn',
    '/sample-net/core/eipAllocationId',
...
async function releaseElasticIPs(): Promise<void> {
  ...
  try {
    ...
    const sample-netEIPs = addresses.Addresses?.filter(
      (addr) => addr.Tags?.some((tag) => tag.Key === 'MAILSERVER' && tag.Value === DOMAIN)
    );

    if (sample-netEIPs && sample-netEIPs.length > 0) {
      for (const eip of sample-netEIPs) {
        if (eip.AllocationId) {
          try {
