...
# Deploy sample-net core stack with custom S3 bucket names
# This allows deployment when buckets with default names already exist in another account
#
# Usage:
#   ./tools/deploy-sample-net-core-with-custom-buckets.sh [backup-bucket-name] [nextcloud-bucket-name]
#
# Example:
#   ./tools/deploy-sample-net-core-with-custom-buckets.sh example-net-backup-v2 example-net-nextcloud-v2

BACKUP_BUCKET_NAME="${1:-example-net-backup-v2}"
NEXTCLOUD_BUCKET_NAME="${2:-example-net-nextcloud-v2}"

echo "🚀 Deploying sample-net core stack with custom bucket names"
echo "════════════════════════════════════════════════════════"
...
if AWS_PROFILE="${AWS_PROFILE:-sample-net}" AWS_REGION="${AWS_REGION:-us-east-1}" \
...
AWS_PROFILE="${AWS_PROFILE:-sample-net}" \
AWS_REGION="${AWS_REGION:-us-east-1}" \
FEATURE_CDK_MAILSERVER_DEMO_ENABLED=1 \
DOMAIN=example.net \
