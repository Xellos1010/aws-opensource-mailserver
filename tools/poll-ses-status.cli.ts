 ...
 * 
 * Usage:
 *   APP_PATH=apps/clients/cdk-client-example/instance DOMAIN=example.com pnpm exec tsx tools/poll-ses-status.cli.ts
 */
...
const coreAppPath = process.env['CORE_APP_PATH'] || 'apps/clients/cdk-client-example/core';
...
async function pollSesStatus() {
  ...
  try {
    ...
    for (const record of expectedRecords) {
      ...
      const mxMatch = normalizedActual.match(/^\d+\s+(.+?)(?:\.mailexample\.com\.?)?$/);
      const actualMxHost = mxMatch ? mxMatch[1].replace(/\.$/, '') : normalizedActual.replace(/\.mailexample\.com\.?$/, '').replace(/\.$/, '');
