...
/**
 * Comprehensive verification and setup script for Sample net mailserver
 * 
...
async function verifyRecoverySystem(options: VerifyOptions): Promise<void> {
  ...
  const profile = options.profile || process.env.AWS_PROFILE || 'sample-net';
...
async function verifySesConfiguration(options: VerifyOptions): Promise<void> {
  ...
  if (allVerified) {
    ...
    if (verificationStatus !== 'Success') {
      ...
      console.log(`      nx run cdk-client-example-instance:admin:ses-dns\n`);
    }
...
async function provideMailboxRestoreGuidance(options: VerifyOptions): Promise<void> {
  ...
  console.log('   BACKUP_PATH=/path/to/backup pnpm nx run cdk-client-example-instance:admin:restore:users-and-mailboxes\n');
  
  console.log('2. Restore mailboxes only:');
  console.log('   BACKUP_PATH=/path/to/backup pnpm nx run cdk-client-example-instance:admin:mailboxes:restore\n');
  
  console.log('3. Restore aggregated mailboxes (from multiple backups):');
  console.log('   BACKUP_FOLDERS=/path1,/path2 pnpm nx run cdk-client-example-instance:admin:mailboxes:restore-aggregated\n');
  
  console.log('4. Discover users from backup:');
  console.log('   BACKUP_PATH=/path/to/backup pnpm nx run cdk-client-example-instance:admin:users:discover-from-mailboxes\n');
...
async function main(): Promise<void> {
  ...
  const profile = process.env.AWS_PROFILE || 'sample-net';
  const appPath = process.env.APP_PATH || 'apps/clients/cdk-client-example/instance';
  const domain = process.env.DOMAIN || 'example.net';

  console.log('🚀 Sample net Mailserver Verification & Setup');
