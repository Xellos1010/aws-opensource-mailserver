...
async function diagnoseWebmail401(options: DiagnoseWebmail401Options): Promise<void> {
  ...
  const testEmail = options.email || process.env.TEST_EMAIL || 'test@example.com';
  ...
  console.log('   4. Verify user password: pnpm nx run cdk-client-example-instance:admin:credentials:sync');
  console.log('   5. Check logs: pnpm nx run cdk-client-example-instance:admin:logs:webmail\n');
}
...
Options:
  ...
  --email, -e <email>        Test email address (default: test@example.com)
