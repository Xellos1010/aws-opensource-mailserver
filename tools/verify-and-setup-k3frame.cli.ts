...
/**
 * Comprehensive verification and setup script for K3 Frame mailserver
 * 
...
async function verifyRecoverySystem(options: VerifyOptions): Promise<void> {
  ...
  const profile = options.profile || process.env.AWS_PROFILE || 'k3frame';
...
async function verifySesConfiguration(options: VerifyOptions): Promise<void> {
  ...
  if (allVerified) {
    ...
    if (verificationStatus !== 'Success') {
      ...
      console.log(`      nx run cdk-k3frame-instance:admin:ses-dns\n`);
    }
...
async function provideMailboxRestoreGuidance(options: VerifyOptions): Promise<void> {
  ...
  console.log('   BACKUP_PATH=/path/to/backup pnpm nx run cdk-k3frame-instance:admin:restore:users-and-mailboxes\n');
  
  console.log('2. Restore mailboxes only:');
  console.log('   BACKUP_PATH=/path/to/backup pnpm nx run cdk-k3frame-instance:admin:mailboxes:restore\n');
  
  console.log('3. Restore aggregated mailboxes (from multiple backups):');
  console.log('   BACKUP_FOLDERS=/path1,/path2 pnpm nx run cdk-k3frame-instance:admin:mailboxes:restore-aggregated\n');
  
  console.log('4. Discover users from backup:');
  console.log('   BACKUP_PATH=/path/to/backup pnpm nx run cdk-k3frame-instance:admin:users:discover-from-mailboxes\n');
...
async function main(): Promise<void> {
  ...
  const domain = process.env.DOMAIN || 'k3-frame.com';

  console.log('🚀 K3 Frame Mailserver Verification & Setup');
