...
Options:
  --app-path PATH          App path used for stack resolution (default: apps/clients/cdk-client-example/instance)
  --domain DOMAIN          Domain name (default: derived from app path; fallback example.com)
...
async function main(): Promise<void> {
  ...
  const appPath =
    cli.appPath || process.env.APP_PATH || 'apps/clients/cdk-client-example/instance';
  const region =
    cli.region ||
    ...
    'us-east-1';
  const profile =
    cli.profile || process.env.AWS_PROFILE || process.env.PROFILE || 'your-aws-profile';
  ...
  try {
    ...
    console.error(`   - pnpm nx run cdk-client-example-instance:admin:bootstrap-miab-ec2-instance -- --domain ${resolvedDomain}`);
    console.error(`   - pnpm nx run cdk-client-example-instance:admin:bootstrap:status -- --domain ${resolvedDomain}`);
    process.exit(1);
  }
