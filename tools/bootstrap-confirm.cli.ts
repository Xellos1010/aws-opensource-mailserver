...
async function confirmBootstrap(options: ConfirmationOptions): Promise<void> {
  ...
  try {
    ...
    if (failed === 0 && warnings <= 2) {
      ...
      console.log('   1. Check bootstrap logs: pnpm nx run cdk-client-example-instance:admin:bootstrap:logs');
      console.log('   2. Check SSM command status: pnpm nx run cdk-client-example-instance:admin:bootstrap:status');
      console.log('   3. SSH to instance and check: /var/log/mailinabox_setup.log');
      console.log('   4. Re-run bootstrap if needed: pnpm nx run cdk-client-example-instance:admin:bootstrap-miab-ec2-instance\n');
      process.exit(1);
    }
  } catch (error) {
