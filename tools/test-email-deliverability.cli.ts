 ...
 * 2. Send test email via SMTP (alice@example.com -> external address)
 * 3. Verify delivery in mail logs
 * 4. Check IMAP authentication
 * 5. Test email receiving (external -> alice@example.com)
 */
...
async function testEmailDeliverability(options: TestOptions): Promise<void> {
  ...
  const domain = options.domain || process.env.DOMAIN || 'example.com';
  const fromEmail = options.fromEmail || process.env.FROM_EMAIL || `alice@${domain}`;
  ...
  const instanceId = options.instanceId || 'i-0b86bbad6a2c66f1d'; // sample mailserver instance
...
Options:
  ...
  --domain, -d <domain>        Domain name (default: example.com)
  --from-email <email>         From email address (default: alice@<domain>)
