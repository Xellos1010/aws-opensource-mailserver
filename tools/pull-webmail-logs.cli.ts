...
async function pullWebmailLogs(options: PullWebmailLogsOptions): Promise<void> {
  ...
  try {
    const authCommand = `tail -n ${lines * 2} /var/log/mail.log /var/log/nginx/access.log /var/log/roundcube/userlogins.log 2>/dev/null | grep -iE "(login|auth|401|unauthorized|test@mailexample)" | tail -n ${lines}`;
  ...
  console.log(`   5. Verify user exists: pnpm nx run cdk-client-example-instance:admin:users:list`);
  console.log(`   6. Check user password: pnpm nx run cdk-client-example-instance:admin:credentials:sync`);
}
