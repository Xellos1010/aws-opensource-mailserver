 ...
 *
 * Run via Nx: pnpm nx run cdk-client-example-instance:admin:verify:anti-spam-dns
 */
...
async function verifyAntiSpamDns(options: VerifyOptions): Promise<CheckResult[]> {
  ...
  const coreAppPath = options.coreAppPath || process.env.CORE_APP_PATH || 'apps/clients/cdk-client-example/core';
...
function printResults(results: CheckResult[]): void {
  ...
  if (fail.length > 0) {
    ...
    console.log(
      '   See: docs/public/mail-server-operations.md (DNS section) or run the SES DNS print target on your instance app (e.g. nx run cdk-client-example-instance:admin:ses-dns:print).\n'
    );
    process.exit(1);
  }
}
...
Options:
  --domain, -d <domain>     Domain (e.g. example.com)
...
`);
  ...
  if (!options.domain) options.domain = 'example.com';
