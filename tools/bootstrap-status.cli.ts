...
async function checkBootstrapStatus(options: StatusOptions): Promise<void> {
  ...
  try {
    ...
    if (!ssmStatus.online) {
      ...
      console.log(`   Run: pnpm nx run cdk-client-example-instance:admin:fix-ssm-agent\n`);
      return;
    }
    ...
    if (commandStatus.status === 'Success') {
      ...
      console.log(`   3. Check SSM agent: pnpm nx run cdk-client-example-instance:admin:fix-ssm-agent`);
