...
async function repairMailDelivery(options: RepairOptions): Promise<RepairReport> {
  ...
  if (!overallSuccess) {
    ...
    console.log('   2. Check logs: pnpm nx run cdk-client-example-instance:admin:logs:dovecot');
    console.log('   3. Run health gate: pnpm nx run cdk-client-example-instance:admin:health-gate');
  } else {
    console.log('💡 Verification:');
    console.log('   1. Test sending email from your mail client');
    console.log('   2. Test receiving email by sending to your address');
    console.log('   3. Run: pnpm nx run cdk-client-example-instance:admin:mail:flow:test');
  }
  console.log('');
...
Examples:
  ...
  pnpm nx run cdk-client-example-instance:admin:repair:mail

  # Repair specific user
  USER_EMAIL=sysops@example.net pnpm nx run cdk-client-example-instance:admin:repair:mail

  # Dry run to preview
  DRY_RUN=1 pnpm nx run cdk-client-example-instance:admin:repair:mail
`);
        process.exit(0);
    }
  }
