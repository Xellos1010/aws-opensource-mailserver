 ...
 * 
 * Usage:
 *   APP_PATH=apps/clients/cdk-client-example/core DOMAIN=example.com pnpm exec tsx tools/print-ses-dns-records.cli.ts
 * 
 * Or use the Nx target:
 *   pnpm nx run cdk-client-example-instance:admin:ses-dns:print
 */
...
const coreAppPath = process.env['APP_PATH'] || 'apps/clients/cdk-client-example/core';
...
async function printDnsRecords() {
  ...
  try {
    ...
    console.log('   - Run: pnpm nx run cdk-client-example-instance:admin:nameserver:audit');
    ...
    console.log('   - Run: pnpm nx run cdk-client-example-instance:admin:ses-dns');
    ...
    console.log('6. Verify locally with: pnpm nx run cdk-client-example-instance:admin:ses:status\n');
