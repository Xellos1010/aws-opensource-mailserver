...
async function main(): Promise<void> {
  const cli = parseArgs();
  const appPath = cli.appPath || process.env.APP_PATH || 'apps/clients/cdk-client-example/instance';
  const region =
    cli.region || process.env.AWS_REGION || process.env.CDK_DEFAULT_REGION || 'us-east-1';
  const profile = cli.profile || process.env.AWS_PROFILE || process.env.PROFILE || 'your-aws-profile';
